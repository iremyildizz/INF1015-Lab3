...
int main()
{
	...
	auto film = listeFilms.chercherFilmlamda([](const Film& f)
		{
			return f.recette == 955;
		});
	if (film != nullptr)
	{
		cout << "Film trouvé ayant 955M$: " << film->titre << endl;
	}

	Liste<string> listeTextes(2);
	listeTextes.elements[0] = make_shared<string>("Hello");
	listeTextes.elements[1] = make_shared<string>("World");

	Liste<string> listeTextes2 = listeTextes;
	listeTextes.elements[0] = make_shared<string>("Bye");
	*(listeTextes.elements[1]) = "People";
